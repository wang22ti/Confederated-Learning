...
if __name__ == '__main__':
    ...
    log_file = open('log/{}_{}_reweighting.log'.format(target_name, noise_rate), 'a')
    for alpha in [0.1 * _ for _ in range(14, 5, -1)]:
        init(0)

        # prepare data
        print('reweighting, target:', target_name, 'noise rate:', noise_rate, 'alpha:', alpha)
        print('reweighting, target:', target_name, 'noise rate:', noise_rate, 'alpha:', alpha, file=log_file)
        ...
        for epoch in range(num_epoch):
            ...
            for data, target in train_loader:
                ...
                with torch.no_grad():
                    ...
                    raw_target = torch.zeros(data.shape[0], 10).cuda().scatter_(1, target.unsqueeze(dim=-1).long(), 1)
