...
def generate_noise(dataset, indices, num_noise=0, num_classes=31):
...
def get_noise_idx(dataset, indices, num_noise=0, num_classes=31):
    random.shuffle(indices)
    ...
    for idx, label in zip(noise_indx, noise_label):
        dataset.imgs[idx] = (dataset.imgs[idx][0], label)
    return dataset, noise_indx

def get_noise_idx2(dataset, indices, num_noise=0, num_classes=31):
    random.shuffle(indices)
...
def get_office(name_dataset):
    root_dir = os.path.join('office', name_dataset)
